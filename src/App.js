...
function App() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [workersRes, clientsRes, brandsRes, productsRes, ordersRes, deliveriesRes, incomeRes, expensesRes, productionRes, qualityRes, dashboardRes] = await Promise.all([
  ...
  return (
    <div style={{ maxWidth: '1400px', margin: '0 auto', padding: '20px', fontFamily: 'Arial, sans-serif' }}>
                ...
                <div key={delivery._id} style={{ 
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <div>
                      ...
                      <p style={{ margin: '5px 0 0 0', color: '#64748b' }}>
                        🚚 {delivery.driver?.name} • Order: {delivery.order?.orderNumber}
                      </p>
                      <p style={{ margin: '5px 0 0 0', color: '#64748b', fontSize: '0.9em' }}>
      ...
      {/* Other tabs (workers, income, expenses) - simplified for now */}
      {['workers', 'income', 'expenses'].includes(activeTab) && (
        <div style={{ background: 'white', padding: '25px', borderRadius: '12px', border: '1px solid #e2e8f0' }}>
          <h2 style={{ color: '#1e293b', margin: '0 0 20px 0' }}>
            {t[activeTab] || activeTab.charAt(0).toUpperCase() + activeTab.slice(1)}
          </h2>
          <p style={{ color: '#64748b', textAlign: 'center', padding: '40px' }}>
            {t[activeTab]} module is ready for implementation. This would include detailed forms and management interfaces.
          </p>
        </div>
      )}
      ...
      <footer style={{ 
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(250px, 1fr))', gap: '30px', marginBottom: '20px' }}>
          <div>
            ...
            <p style={{ margin: '0', fontSize: '0.9em', color: '#94a3b8' }}>
              Complete FMCG distribution management for Algerian market
            </p>
          </div>
          <div>
            <h3 style={{ margin: '0 0 10px 0', color: '#10b981' }}>📊 System Features</h3>
            <p style={{ margin: '0', fontSize: '0.9em', color: '#94a3b8' }}>
              ✅ Pre-sales system<br/>
          ...
          <div>
            <h3 style={{ margin: '0 0 10px 0', color: '#8b5cf6' }}>🇩🇿 Market Ready</h3>
            <p style={{ margin: '0', fontSize: '0.9em', color: '#94a3b8' }}>
              🇩🇿 Algeria Market<br/>
              💵 DZD Currency<br/>
              🇫🇷/🇬🇧 Bilingual<br/>
        ...
        <div style={{ 
          ...
          <p style={{ margin: '0 0 10px 0' }}>
            © 2024 {t.title}. Transforming FMCG distribution in Algeria, one delivery at a time.
          </p>
          <p style={{ margin: '0' }}>
            🚚 Built for Algerian distributors | 
            💼 Investor-ready platform | 
            🌍 Mobile-optimized for sales agents
          </p>
        </div>
      </footer>
    </div>
